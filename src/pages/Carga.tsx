...
import MedioPagoGastoFields, { type MedioPagoGastoFieldsProps, type MedioPagoGastoNivel } from '../components/MedioPagoGastoFields'
import MedioPagoSuscripcionFields, { type MedioPagoSuscripcionFieldsProps } from '../components/MedioPagoSuscripcionFields'
...
import { transaccionSchema, compraCuotasSchema, parseFormData } from '../lib/schemas'
...
export default function Carga() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if ((esTarjetaCreditoGasto || esTarjetaCreditoSuscripcion) && enCuotas) {
      ...
      const parsed = parseFormData(compraCuotasSchema, cuotaPayload)
      if (!parsed.success) {
        notify.warning(parsed.errors.join(' · '))
        return
      }
      setSubmitting(true)
      try {
        await insertCuota(parsed.data)
        notify.success('Cuotas registradas')
    ...
    const parsed = parseFormData(transaccionSchema, txPayload)
    if (!parsed.success) {
      notify.warning(parsed.errors.join(' · '))
      return
    }
    ...
    try {
      ...
      if (result.offline) {
        notify.info('Guardado localmente. Se sincronizará cuando recuperés conexión.')
      } else {
      ...
      const hintTransferencia =
        ...
        (err.code === '23514' || /medio_pago|check constraint/i.test(err.message ?? ''))
          ...
          : ''
      notify.error('Error: ' + (err.message ?? 'Error desconocido') + hintTransferencia)
    } finally {
      setSubmitting(false)
    }
  }
